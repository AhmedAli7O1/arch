...
class NodeArch extends EventEmitter {
  ...
  async init () {
    ...
    if (this.arch.logger)
      this.logger = createLogger(this.arch.logger);
  ...
  async start (serverHandler) {
    try {
      ...
      this.logger.info(`Node.js Server/Architecture Manager v${this.pkgInfo.version}`);
      this.logger.info("http://www.nodearch.io");
      this.logger.info(`Environment >> ${this.env}`);
      this.logger.info(`Started In ${this.paths.app}`);
      this.logger.info(`Started At >> ${Date()}`);
